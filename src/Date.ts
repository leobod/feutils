 ...
 */
const formatDate = function (date: DateType, format: string): string {
...
export const getStartOfHour = (d, h = 'HH') => formatDate(d, `YYYY/MM/DD ${h}:00:00`)
...
export const getCloserDateTime = (d, interval = 15, step = 0, type = 'ceil') => {
  const date = toDate(d)
  let numberInterval
  if (type === 'ceil') {
    /* 取上整 */
